...
| **阅读进度条** | 顶部彩色进度指示 |
| **框选菜单** | Win11 风格应用内浮动药丸随选区滚动跟随：复制、自由收藏、AI 翻译/解释、应用内搜索、站内/网页搜索、简繁转换 |

### 🤖 AI 助手

> 完全由用户自行配置:应用**不内置任何 AI 服务与密钥**,须先在「我的 → AI 设置」中添加供应商、启用功能并打开总开关,阅读页才会出现 AI 入口。

| 功能 | 说明 |
|:---|:---|
| **多协议供应商** | OpenAI 兼容接口 / Anthropic Claude / Google Gemini,可添加多个 |
| **完全自定义** | API 地址、密钥、模型、温度、最大输出、自定义请求路径(支持 `{model}` 占位)、自定义请求头,均可配置 |
| **流式输出** | SSE 逐字渲染,可随时停止;不兼容端点可切换整段返回 |
| **内置功能** | 内容摘要 / 翻译 / 名词解释 / 自由对话,提示词全部可改 |
| **自定义功能** | 任意添加快捷指令,绑定供应商与模型,`{title}` `{content}` 占位符注入当前文档 |
| **阅读上下文** | 自动携带当前文档纯文本,携带字数上限可调(1000-30000 字) |
| **框选直达** | 阅读页框选文字即可对选段发起 AI 翻译/名词解释 |
| **主题跟随** | 对话页配色与当前阅读主题(浅色/护眼/深色/纯黑)一致 |
| **连接测试** | 供应商保存前可一键测试连通性 |
| **本地保存** | 所有配置(含密钥)仅存于本机,请求只发往你填写的地址 |
...
| **备份与恢复** | 个性化数据一键备份为 JSON 文件：偏好设置、AI 配置、自由收藏、点赞/已读、历史/待读、阅读位置、草稿；恢复支持按类目勾选与合并/替换 |
| **自由收藏** | 阅读时框选文字一键收藏，每条独立成框、框内文字可选取，支持复制/删除/撤销 |
